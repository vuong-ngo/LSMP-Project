# ============================================================================
# file: src/lsmp_ai/scripts/export_models_to_db.py
# Description: Script to export all registered AI model metadata, evaluation metrics,
#              and anomaly prediction results to PostgreSQL / TimescaleDB.
# ============================================================================

import os
import sys
import json
import argparse
from pathlib import Path
import pandas as pd

# Ensure src/ is in sys.path
BASE_DIR = Path(__file__).resolve().parent.parent.parent.parent
if str(BASE_DIR / "src") not in sys.path:
    sys.path.insert(0, str(BASE_DIR / "src"))

from lsmp_ai.common.logger import setup_logger
from lsmp_ai.io.db_client import DBClient

logger = setup_logger(__name__)


def export_models_and_metrics_to_db(db_url: str = None) -> bool:
    """Exports all registered model catalog versions and evaluation metrics to the database."""
    logger.info("Initializing Database Export Utility for LSMP AI Models...")
    client = DBClient(db_url=db_url)

    if not client.db_url:
        logger.warning("DATABASE_URL is not set. Please export DATABASE_URL variable.")
        return False

    models_dir = BASE_DIR / "models_store"
    catalog_path = models_dir / "catalog.json"

    if not catalog_path.exists():
        logger.warning(f"No catalog.json found at {catalog_path}.")
        return False

    with open(catalog_path, "r") as f:
        catalog = json.load(f)

    exported_count = 0
    for version, info in catalog.items():
        if version == "latest_version":
            continue

        metrics = info.get("metrics", {})
        run_id = f"export_{version}"

        try:
            client.write_evaluation_metrics(
                run_id=run_id,
                model_config_name="cascade_iforest_ocsvm",
                dataset_split="cicids2017",
                precision=metrics.get("precision", 0.0),
                recall=metrics.get("recall", 0.0),
                f1=metrics.get("f1_score", 0.0),
                fpr=metrics.get("false_positive_rate", 0.0),
                roc_auc=metrics.get("roc_auc", 0.5),
                hyperparameters=info.get("hyperparameters", {}),
                model_version=version,
            )
            exported_count += 1
        except Exception as err:
            logger.error(f"Failed to export metrics for version {version}: {err}")

    # Also export fallback prediction results if interim predictions exist
    predictions_file = BASE_DIR / "data" / "interim" / "predictions_fallback.csv"
    if predictions_file.exists():
        try:
            df_preds = pd.read_csv(predictions_file)
            if not df_preds.empty and "score" in df_preds.columns:
                from lsmp_ai.io.result_writer import ResultWriter
                writer = ResultWriter(db_client=client)
                count = writer.write_predictions(df_preds)
                logger.info(f"Successfully wrote {count} risk score predictions to database.")
        except Exception as pred_err:
            logger.warning(f"Could not export prediction results: {pred_err}")

    return exported_count > 0


def main():
    parser = argparse.ArgumentParser(description="Export LSMP Model Metadata & Metrics to Database")
    parser.add_argument("--db-url", "-d", type=str, default=None, help="Database connection URL")
    args = parser.parse_args()

    success = export_models_and_metrics_to_db(db_url=args.db_url)
    if not success:
        sys.exit(1)


if __name__ == "__main__":
    main()
