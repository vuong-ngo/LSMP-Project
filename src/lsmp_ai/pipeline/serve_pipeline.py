...
def run_serve_pipeline(
    ...
    # IF NO NEW LOGS EXIST: Return immediately cleanly (No stale data recalculation!)
    if df_raw.empty:
        logger.info("🟢 No new raw logs found in the database for current window. Service idle & resting (No duplicate Risk Score recalculation).")
        # Update watermark to end_time so we don't query empty interval again
    ...
    if df_features.empty:
        logger.info("No feature vectors generated from raw logs.")
        save_last_watermark(end_time)
        return pd.DataFrame()
